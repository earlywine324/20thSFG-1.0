...
export async function approveApplication(id: string) {
  ...
  // Auto-create a candidate profile in the LIONHEART screening pipeline
  ...
  const { error: soldierError } = await supabase.from("soldiers").insert({
    ...
    mos_title: app.mos_preference === "any" ? "LIONHEART Candidate" : getMosTitle(app.mos_preference),
    role: "LIONHEART Candidate",
    unit: "LIONHEART Selection",
    team: "Candidate Screening",
    status: "ACTIVE DUTY",
    ...
    awards: [],
    service_record: [{ date: enlistDate, event: "Accepted", details: "Accepted into LIONHEART candidate screening, Interagency Special Missions Group." }],
    rank_history: [{ rank: "PVT", rankFull: "Private", date: enlistDate, authority: "Enlistment" }],
    assignment_history: [{ position: "LIONHEART Candidate", unit: "LIONHEART Selection", dateFrom: enlistDate }],
  });
  ...
  if (!soldierError && app.billet_id) {
    ...
    if (assignResult.error) {
      ...
      // Non-fatal: candidate remains in LIONHEART screening as fallback
    }
  }
...
function getMosTitle(mos: string): string {
  ...
  return titles[mos] || "LIONHEART Candidate";
}
